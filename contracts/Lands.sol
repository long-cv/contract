...
contract Lands is ERC165, ILands, Pausable {
    ...
    mapping(address => Tokens[]) internal _ownerTokens; // array of tokenId of a owner
    ...
    mapping(string => uint256) internal _tokenSupplies; // number of each tokenId supplied
    ...
    function transferFrom(address from, address to, string memory tokenId, uint256 amount) public override whenNotPaused {
        ...
        _ownerTokens[from][index].timestamp = uint64(block.timestamp % 2**64);
        ...
        if (!_isOwners[to][tokenId]) {
            ...
            _ownerTokens[to][index].timestamp = uint64(block.timestamp % 2**64);
        }
    ...
    ///   (sort order not specified)
    function tokenOfOwnerByIndex(address owner, uint256 index) external override view returns (Tokens memory) {
    ...
    function getTokensOfOwner(address owner) external override view returns(Tokens[] memory) {
    ...
    function getTokenTimestamp(address owner, string memory tokenId) external override view returns(uint64) {
        require(isValidToken(tokenId), "getTokenTimestamp: token id is invalid.");
        require(isOwnerOf(owner, tokenId), "getTokenTimestamp: requrest for address not be an owner of token");

        uint256 index = _ownerTokenIndexes[owner][tokenId];
        return _ownerTokens[owner][index].timestamp;
    }
    ...
    function updateTokenTimestamp(address owner, string memory tokenId, uint64 newTimestamp) external override whenNotPaused {
        ...
        require(isOwnerOf(owner, tokenId), "updateTokenTimestamp: requrest for address not be an owner of token");

        uint256 index = _ownerTokenIndexes[owner][tokenId];
        _ownerTokens[owner][index].timestamp = newTimestamp;
    }
