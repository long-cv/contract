...
contract Manager is IManager {
    ...
    constructor(address creator, address time, address landType, address land, uint256 landPrice, uint256 upgradableBasePrice, uint64 intervalKind) {
        _creator = creator;
        _time = time;
        _land = land;
        _landType = landType;
    ...
    function buyLand(string memory quadkey, uint176 amount) external override {
        require(ILand(_land).isOwnerOf(msg.sender, quadkey), "Manager >> buyLand: not owner");
        uint256 price = _landPrice.mul(amount);

        safeTransferToSupplier(_time, msg.sender, price);
       ILand(_land).issueToken(msg.sender, quadkey, amount);
    }

    function getUpgradableLandPrice(uint16 fromLandId, uint16 toLandId, uint256 amount) external override view returns(uint256) {
        uint64 fromInterval = ILandType(_landType).getTokenInterval(fromLandId);
        uint64 toInterval = ILandType(_landType).getTokenInterval(toLandId);
    ...
    function upgradeLand(string memory quadkey, uint16 fromLandId, uint16 toLandId, uint176 amount) external override {
        require(ILand(_land).isOwnerOf(msg.sender, quadkey), "Manager >> upgradeLand: not own this quadkey yet");
        ...
        safeTransferToSupplier(_time, msg.sender, price);
        ILandType(_landType).upgradeLand(msg.sender, quadkey, fromLandId, toLandId, amount);
    }
    ...
    function transferLandFrom(address from, address to, string memory quadkey, uint16 landId, uint176 amount) public override {
        ...
        ILand(_land).transferFrom(from, to, quadkey, landId, amount);
    }
    ...
    function computeTimeRewards(LandInfo memory land, uint64 timestamp) private view returns(uint256 reward, uint64 timeElapsed, uint64 timeLeft) {
        if (land.amount > 0) {
            ...
            uint64 interval = ILandType(_landType).getTokenInterval(land.id);
    ...
    function getLandIntervalList() external override view returns(string memory intervals) {
        uint16[] memory landIDs = ILandType(_landType).getTokenIDs();
        intervals = "";
        for (uint i = 0; i < landIDs.length; i++) {
            uint64 interval = ILandType(_landType).getTokenInterval(landIDs[i]);
            if (i == 0) {
    ...
    function getLandList(address owner) external override view returns(string memory list) {
        ...
        string[] memory quadkeyList = ILand(_land).getTokensOfOwner(owner);
        bool first = true;
        list = "";
        for (uint i = 0; i < quadkeyList.length; i++) {
            uint256 amount = ILand(_land).getTokenAmountOfOwner(owner, quadkeyList[i]);
            if (first) {
            ...
            for (uint j = 0; j < landIDs.length; j++) {
                if (ILandType(_landType).isOwnerOf(owner, quadkeyList[i], landIDs[j])) {
                    uint256 index = ILandType(_landType).tokenIndexOfOwnerById(owner, quadkeyList[i], landIDs[j]);
                    LandInfo memory land = ILandType(_landType).tokenOfOwnerByIndex(owner, quadkeyList[i], index);
    ...
    function claimLandReward(address owner, string memory quadkey, uint16 landId) public override {
        bool isOwner = ILandType(_landType).isOwnerOf(owner, quadkey, landId);
        require(isOwner, "Manager >> claimRewardOfLand: not own this land.");
        
        uint256 index = ILandType(_landType).tokenIndexOfOwnerById(owner, quadkey, landId);
        LandInfo memory land = ILandType(_landType).tokenOfOwnerByIndex(owner, quadkey, index);
        ...
        if (reward > 0) {
            ...
            ILandType(_landType).updateTokenTimestamp(owner, quadkey, landId, tokenTimestamp);
    ...
    function claimQuadkeyReward(address owner, string memory quadkey) public override {
        bool isOwner = ILand(_land).isOwnerOf(owner, quadkey);
        require(isOwner, "Manager >> claimRewardOfQuadKey: not own this quadkey");

        LandInfo[] memory landType = ILandType(_landType).getTokensOfOwner(owner, quadkey);
        for (uint i = 0; i < landType.length; i++) {
            if (landType[i].amount > 0) {
                claimLandReward(owner, quadkey, landType[i].id);
            }
        }
    }
